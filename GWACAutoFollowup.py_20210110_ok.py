 ...
 #  follow-up info 
    ...
    def autoFollowUp(self):
        ...
        for sciObj in sciObjs:
            ...
            if fadingslope > 3:
                tmsg = "Stop the obsevations. It is possible a fake transient for %s with the fading slope of %.2f mag per minutes \n" \
...
#==================================================                        
                ...
                #    status = 2
                    ...
                    if fupRecordN.shape[0]>0 and fupRecordN1.shape[0]>0:
                        ...
                        if diffMinutes < self.maxMonitorTime:         
                            if status == 2:
                                if fupRecordN1[1] >16.0:
                                    # higher variation for dim source, set magDiff=0.3
                                    magDiff = magDiff-0.1  
                                ...
                                if magDiff>=self.stageNMagDiff1 and Diff_all >=self.deltaMagDiffTotal:     
                                    ...
                                    if diffMinutes>0:
                                        if fupRecordN[1] >= 15.0:    #for a typical dMe, B-R>2.0, so the mag in B band is lager than 17 mag. difficalt to obs. 
                                            tobs=[{'filter':['R'],'expTime':30,'frameCount':3}]
                                        else:
                                            tobs=[{'filter':['B'],'expTime':30,'frameCount':1},
                                               {'filter':['R'],'expTime':30,'frameCount':3}
                                               ]
                                    ...
                                    if magDiffK>=self.stageNMagDiff2:
                                       ...
                                       # coloraccess = sciObj[8]-sciObj[7]
                                        ...
                                        if status == 3  and Diff_all < self.stage2MagDiff:    
                                            ...
                                            self.sendTriggerMsg005("stop observation for %s, for the small brightness changing of %5.2f in the whole observations. "%(ot2Name, fupRecordN[1]-sciObj[4]))
                                            self.log.warning("%s, %.2f exceed max monitor time(%dminutes), close monitor."%(ot2Name, diffMinutes, self.maxMonitorTime))
                                            break 
